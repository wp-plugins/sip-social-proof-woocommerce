...
define('SP_UTM_CAMPAIGN', 'sip_social_proof' );
...
class SIP_Social_Proof_WC_Admin {
	...
	public function __construct() {
        ...
        add_action( 'admin_menu', array( $this, 'sip_sp_add_setting_page' ), 20 );
        add_filter( 'plugin_action_links_' . SIP_SP_BASENAME, array( $this, 'sip_sp_action_links' ) );
	}
     ...
     * @since 1.0.0
     */
    public function sip_sp_action_links( $links ) {
        $plugin_links = array(
            '<a href="' . admin_url( 'admin.php?page=sip-social-proof-settings' ) . '">' . __( 'Settings', 'sip-social-proof' ) . '</a>'
        );

        $plugin_links[] = '<a target="_blank" href="https://shopitpress.com/docs/' .SIP_SP_PLUGIN_SLUG. '/?utm_source=wordpress.org&utm_medium=SIP-panel&utm_content=v'. SIP_SP_PLUGIN_VERSION .'&utm_campaign=sip_social_proof">' . __( 'Docs', 'sip-social-proof' ) . '</a>';

        if( ! class_exists( 'SIP_WC_Social_Proof_Pro' ) ) {
            $plugin_links[] = '<a target="_blank" href="' .SIP_SP_PLUGIN_PURCHASE_URL. '?utm_source=wordpress.org&utm_medium=SIP-panel&utm_content=v'. SIP_SP_PLUGIN_VERSION .'&utm_campaign=sip_social_proof">' . __( 'Premium Version', 'sip-social-proof' ) . '</a>';
        }
    ...
    public function sip_sp_custom_admin_menu() {
       ...
       //add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $position );
        $this->hook = add_menu_page( 
            ...
            'sip_plugin_panel', 
            NULL,
            SIP_SP_URI . 'admin/assets/images/icon.png',
            62.25             
        );
     ...
     public function sip_sp_remove_duplicate_submenu_page() { 
        /* === Duplicate Items Hack === */
        remove_submenu_page( 'sip_plugin_panel', 'sip_plugin_panel' );
    }

    public function sip_sp_add_setting_page() {
        $args = array(
        ...
        if ( ! empty( $parent ) ) {
            add_submenu_page( $parent , 'Social Proof', 'Social Proof', 'manage_options', 'sip-social-proof-settings', 'sip_sp_settings_page_ui' );
            add_submenu_page( $parent , 'ShopitPress Extras', '<span style="color:#FFFF00">ShopitPress Extras</span>', 'manage_options', 'sip-extras', array( $this, 'sip_sp_admin_menu_ui' ) );                
            add_submenu_page( $parent , 'Plugin Support', 'Plugin Support', 'manage_options', 'http://shopitpress.com/community/', '' );
        } else {
            add_menu_page( $args['page_title'], $args['menu_title'], $args['capability'], $args['page'], array( $this, 'sip_sp_admin_menu_ui' ), NULL , 62.25 );
        }
        /* === Duplicate Items Hack === */
        $this->sip_sp_remove_duplicate_submenu_page();
    ...
    public function sip_sp_admin_assets() {
        wp_register_style( 'sip_sp_custom_wp_admin_css', esc_url( SIP_SP_URI .   '/admin/assets/css/admin.css', false, '1.0.0' ) );
        wp_enqueue_style( 'sip_sp_custom_wp_admin_css' );
    }
     ...
     * @since 1.0.0
     */
    public function sip_sp_admin_menu_ui() {
